# Changelog

All notable changes to this project will be documented in this file.
See [Conventional Commits](https://conventionalcommits.org) for commit guidelines.

## [2.0.10](https://github.com/allohamora/release-test/compare/v2.0.9...v2.0.10) (2026-08-08)

### Documentation

- Add 123 in README.md ([1b03b93](https://github.com/allohamora/release-test/commit/1b03b931df486e371c0dced46ab27a10cdb11651))

### Other

- Revert "docs: add 123 in README.md" ([bf3bb08](https://github.com/allohamora/release-test/commit/bf3bb08833442e4d1dbfcb8bac35dad2b991612e))

## [2.0.9](https://github.com/allohamora/release-test/compare/v2.0.8...v2.0.9) (2026-08-08)

### Build System

- Add 1 to README.md ([7212039](https://github.com/allohamora/release-test/commit/7212039cdd0d55018b50ecb71481535b502c959a))

### Continuous Integration

- Add tabs to if block ([e0a8177](https://github.com/allohamora/release-test/commit/e0a8177d156743f30f411e3c9d5ae66323847b0f))

### Miscellaneous Chores

- Improve changelog ([d25c66f](https://github.com/allohamora/release-test/commit/d25c66f68c43620fb70f624fa36336fb7150729a))

## [2.0.8](https://github.com/allohamora/release-test/compare/v2.0.7...v2.0.8) (2026-07-07)

### Continuous Integration

- Add limit release notes size ([9bc6d8f](https://github.com/allohamora/release-test/commit/9bc6d8f45f042bd22b0e23982bb135dbce0df383))
- Remove anchor in release note limit ([8b4de36](https://github.com/allohamora/release-test/commit/8b4de369223357b798a18a1d517b42b87ea3c4e3))

## [2.0.7](https://github.com/allohamora/release-test/compare/v2.0.6...v2.0.7) (2026-07-02)

### Bug Fixes

- Add touch CHANGELOG.md to release ([0105a38](https://github.com/allohamora/release-test/commit/0105a38a0316ddb084ee718923f2c3a5b16d9f9e))

## [2.0.6](https://github.com/allohamora/release-test/compare/v2.0.5...v2.0.6) (2026-07-02)

### Documentation

- Add 58910 to README.md ([26033c5](https://github.com/allohamora/release-test/commit/26033c5fbbf3119dcd26f83772e262338e8342be))

### Continuous Integration

- Restrict release to default branch ([35e5515](https://github.com/allohamora/release-test/commit/35e5515d184860121674db8249ca5498e6ab05a2))

## [2.0.5](https://github.com/allohamora/release-test/compare/v2.0.4...v2.0.5) (2026-07-02)

### Documentation

- Add 689 to README.md ([a85477c](https://github.com/allohamora/release-test/commit/a85477cfa84c9260193b00ade55ed9578827757c))
- Add test1 to README.md ([be8af0e](https://github.com/allohamora/release-test/commit/be8af0e5b9973d16e35b82c789deaed1163f171a))

### Continuous Integration

- Add skipping for merge branch commits ([58a301a](https://github.com/allohamora/release-test/commit/58a301a377f0faca7bffb05f03e0729e28ef0330))

## [2.0.4](https://github.com/allohamora/release-test/compare/v2.0.3...v2.0.4) (2026-07-02)

### Documentation

- Add 123 to README.md ([660b0b5](https://github.com/allohamora/release-test/commit/660b0b5e15c7c178f50a9435250ea863460b05e0))

### Other

- Revert "docs: add 123 to README.md" ([3eea4b9](https://github.com/allohamora/release-test/commit/3eea4b95d7744fe87e35e39f298f64de1faa761d))

## [2.0.3](https://github.com/allohamora/release-test/compare/v2.0.2...v2.0.3) (2026-07-02)

### Continuous Integration

- Fix wrong changes in changelog ([921437e](https://github.com/allohamora/release-test/commit/921437ee114c4b04616e5d78f237a02ce6fb40ab))

## [2.0.2](https://github.com/allohamora/release-test/compare/v2.0.1...v2.0.2) (2026-06-25)

### Documentation
- Update README.md ([6e4e76d](https://github.com/allohamora/release-test/commit/6e4e76d199bf066adab539dd479bc1c420421007))
- Update README.md again ([08e2ab5](https://github.com/allohamora/release-test/commit/08e2ab563e900044b158ddb84747ca2af21be1cf))

### Chores
- Add correct docs matching ([7afb28d](https://github.com/allohamora/release-test/commit/7afb28dbbd1a210a376c65f7e72182be41e9e715))

## [2.0.1](https://github.com/allohamora/release-test/compare/v2.0.0...v2.0.1) (2026-06-16)

### Continuous Integration
- Remove chore(scope) skips ([d34756d](https://github.com/allohamora/release-test/commit/d34756d994ca40fec7254e0f447a1ca495680d27))
- Remove security group ([7d34d3c](https://github.com/allohamora/release-test/commit/7d34d3c446d39b4eb05f413389345cf6bdf76faa))
- Move commit link after migration guide ([5302fb7](https://github.com/allohamora/release-test/commit/5302fb7e7caab21292ab6f71cf11b200c4c946d2))

## [2.0.0](https://github.com/allohamora/release-test/compare/v1.0.0...v2.0.0) (2026-06-15)

### ⚠ BREAKING CHANGES
- Fix incorrect breaking change handling → a lot of text a lot of text a lot of text a lot text a lot of text a lot of text ([aae0c28](https://github.com/allohamora/release-test/commit/aae0c28e0759f8fe0271d6846ab43fa581c349b1))

## [1.0.0](https://github.com/allohamora/release-test/compare/v0.1.4...v1.0.0) (2026-06-15)

### ⚠ BREAKING CHANGES
- Add breaking change parsing → we parse breaking change now ([9f919fb](https://github.com/allohamora/release-test/commit/9f919fbd984002efd94119a6465ea182c2d05a48))
- Add mutlibranch support ([f9ab8c8](https://github.com/allohamora/release-test/commit/f9ab8c83e5046e5c10e8b07d0f81b29e5d470ff3))

### Continuous Integration
- Add tag already exist check ([2dcb817](https://github.com/allohamora/release-test/commit/2dcb817ee3258f6fe9158949e2a6711c5fe4f02c))

## [0.1.4](https://github.com/allohamora/release-test/compare/v0.1.3...v0.1.4) (2026-06-15)

### Continuous Integration
- Fix wrong revert commit rendering ([a90588c](https://github.com/allohamora/release-test/commit/a90588c81a10524787d63119a66d663320d8cc42))
- Fix spaces issue in changelog ([c72ae39](https://github.com/allohamora/release-test/commit/c72ae3931506d29165e37ff59e9dcaa0c3069059))

## [0.1.3](https://github.com/allohamora/release-test/compare/v0.1.2...v0.1.3) (2026-06-15)

### Continuous Integration
- Remove missing space between releases ([7a485dc](https://github.com/allohamora/release-test/commit/7a485dc15c2f7c7e51d294703003b3e15a95a77a))
- Replace remote github with repo url ([ad174c3](https://github.com/allohamora/release-test/commit/ad174c328b6d15f2b5ae3c36d94342419c9f4060))

### Other
- Revert "ci: add release[bot] name" ([8df7c97](https://github.com/allohamora/release-test/commit/8df7c977937539fac0a0bae3fcc3e97afb48d194))

## [0.1.2](https://github.com/allohamora/release-test/compare/v0.1.1...v0.1.2) (2026-06-15)

### Continuous Integration
- Update release notes generation to remove title header ([7ff992a](https://github.com/allohamora/release-test/commit/7ff992ae95d89f74eaa11aaab5eed3041f416ea1))
- Add release[bot] name ([4eaeaa8](https://github.com/allohamora/release-test/commit/4eaeaa8f069ed2ef66cfcd8b2938ae8fd865b7ca))

## [0.1.1](https://github.com/allohamora/release-test/compare/v0.1.0...v0.1.1) (2026-06-15)

### Continuous Integration
- Update breaking changes label to include warning emoji ([c08c662](https://github.com/allohamora/release-test/commit/c08c662db27d374bb86c823bb3424d59dcf7cbd6))

## 0.1.0 (2026-06-15)

### ⚠ BREAKING CHANGES
- Add 4 ! ([8cacb65](https://github.com/allohamora/release-test/commit/8cacb6583efca5936ef54d761f4ee8d506c27ed5))
- Add 5 ! ([4cee446](https://github.com/allohamora/release-test/commit/4cee446a7805738c747c2c2a5ea0e9b6877cd92d))
- Add 6! ([ab4df7b](https://github.com/allohamora/release-test/commit/ab4df7bee941e90fab0ea91b391984ab9e928f9f))
- Add third ! to hello world ([9675895](https://github.com/allohamora/release-test/commit/9675895c606cedb1c384c22bf3599c168d25ce00))
- **api:** Add 4 ! to hello world comment → now we have 4 ! in the comment ([9219291](https://github.com/allohamora/release-test/commit/921929140f58c89bc3126340942e0a04c79055f3))

### Features
- **api:** Add extra! ([cbb2c4f](https://github.com/allohamora/release-test/commit/cbb2c4fba23bca6741796e91f1505722ec402f84))
- Clear everything ([0d42009](https://github.com/allohamora/release-test/commit/0d42009b7c8438dc3a3925c16347173015587701))

### Bug Fixes
- Add third! ([c5a0b2e](https://github.com/allohamora/release-test/commit/c5a0b2e7463dcfa88d3fcf8e56b195afd598163b))

### Refactor
- Add !! to the comment ([1447cb1](https://github.com/allohamora/release-test/commit/1447cb18cece60396939958e2545db6187104e32))

### Documentation
- Add hello world comment ([f51aacd](https://github.com/allohamora/release-test/commit/f51aacda8a7ed284443adc50d5200c4e25a2a517))

### Continuous Integration
- Add git-cliff ([9983e94](https://github.com/allohamora/release-test/commit/9983e94128700528e5b2897c452aecbe463393f2))
- Add github integration to cliff ([144c1e0](https://github.com/allohamora/release-test/commit/144c1e04cb255ae656c8823d84e8f2cafa730f57))
- Remove order comments from git-cliff ([f9a56a6](https://github.com/allohamora/release-test/commit/f9a56a682b44a15bedebe11453912c6702e2b65d))
- Add conventional changelog like format ([c798c25](https://github.com/allohamora/release-test/commit/c798c25db87a7c42774d7c76eecacfe884ebfa71))
- Remove emojies and add caps to breaking changes ([e11cafb](https://github.com/allohamora/release-test/commit/e11cafb8b2cbec5e222e269b6cc1824549f88965))
- Split chore and ci ([4d2160a](https://github.com/allohamora/release-test/commit/4d2160a99e9ea97c1e3817bbd2c90c92c09957be))
- Remove link parsers ([0702f36](https://github.com/allohamora/release-test/commit/0702f367184d92812c02f7ec889a38423a194c32))
- Skip merge pull request messages in changelog ([409352f](https://github.com/allohamora/release-test/commit/409352fdaeb0a21009ea6b8aecb627034b3a15ab))
- Remove spaces and unify scopes ([a5d8a0a](https://github.com/allohamora/release-test/commit/a5d8a0a57f93d662d67cbc3dd1ecdec75ffce82b))
- Return Breaking Changes group ([2db5ae6](https://github.com/allohamora/release-test/commit/2db5ae6d514ae78665af0b241785b3a03eea4d48))
- Add tags comparison ([98272f7](https://github.com/allohamora/release-test/commit/98272f747447f1a57684ac2888957fea4fbffdbb))
- Add header ([935ca1d](https://github.com/allohamora/release-test/commit/935ca1dd3f433b2abaabe19c3cdefa1de7ce3d54))
- Skip release version commits in changelog ([9cd05f1](https://github.com/allohamora/release-test/commit/9cd05f1317fadcf7eff8088315477daccc629ea5))
- Add release workflow ([65db29d](https://github.com/allohamora/release-test/commit/65db29d021f3c2d8f43c880907f3141c536c7148))
- Add GH_TOKEN ([2f99511](https://github.com/allohamora/release-test/commit/2f9951109a53a817123240089e9c0931e7767811))
- Make BREAKING CHANGES ([f69ea37](https://github.com/allohamora/release-test/commit/f69ea37f71e7c694e47f220d5916ccdfc0d5f11d))
- Remove release pull request ([9dd6d65](https://github.com/allohamora/release-test/commit/9dd6d6536fd3908b47bb86f73abb084cfad13a40))
- Update release version pattern in commit parser ([b5c3d9c](https://github.com/allohamora/release-test/commit/b5c3d9c3f3bda12720d494aafeb0d414dadf3146))
- Add release notes ([eb8317e](https://github.com/allohamora/release-test/commit/eb8317ecc853a4a166c58554554cacaf971e3cf9))
- Add --no-install ([f6e65e3](https://github.com/allohamora/release-test/commit/f6e65e3c12bf3f473c17ce241f732c5030855008))
- Bump version in package.json ([db93f19](https://github.com/allohamora/release-test/commit/db93f198d6155223e12f53b7a0b50d4fa1308770))
- Update release notes generation and commit process ([1238bc7](https://github.com/allohamora/release-test/commit/1238bc73179b0c9f09f4933fc66abd543af6edf9))
- Remove GH_TOKEN environment variable from release creation step ([37b430a](https://github.com/allohamora/release-test/commit/37b430aff1894caf1bccac2d00526bb05645e158))
- Change workflow name from 'Release' to 'release' ([86d081c](https://github.com/allohamora/release-test/commit/86d081c8f31c2d4d04d65908d8fcd43e9d500bc2))
- Generate release notes after commit ([1b5ef2f](https://github.com/allohamora/release-test/commit/1b5ef2ff307850ea8ac3b1c6ab53750e754e7133))

### Chores
- Add base ([38e2ef4](https://github.com/allohamora/release-test/commit/38e2ef4ba2a7f7cb46353804e2cc105f33ccce74))
- Add husky and commitlint ([148964b](https://github.com/allohamora/release-test/commit/148964b3be9a161bdd2b2e1d31a204ae38f83b93))

### Other
- Initial commit ([4c57469](https://github.com/allohamora/release-test/commit/4c5746918c2e14f137597bd0241cb45b8f4ac4eb))
- Revert "ci: remove GH_TOKEN environment variable from release creation step" ([55ca69c](https://github.com/allohamora/release-test/commit/55ca69c64fb0cfacc836c5bc3809540e6deef65b))
